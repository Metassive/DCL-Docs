...
def eliminar_etiquetas_time(texto):
    return re.sub(r'<relative-time.*?>.*?</relative-time>', '', texto, flags=re.DOTALL | re.IGNORECASE)

def reemplazar_saltos_de_linea(texto):
    return re.sub(r'\n+', '\n\n', texto)

with open('github-decentraland-scenes-TEXT.txt', 'r', encoding='ISO-8859-1') as archivo:
...
for i, parte in enumerate(partes_divididas, start=1):
    with open(f'github-decentraland-scenes-TEXT-0{i}---clean.txt', 'w', encoding='utf-8') as archivo:
        archivo.write('\n\n'.join(parte))
